...
const { enable3d, Scene3D, ExtendedObject3D, THREE, ThirdPersonControls, PointerLock, PointerDrag  } = ENABLE3D;
...
export default class MainScene extends Scene3D {
    ...
    create() {
            ...
            if (!isTouchDevice) {
                ...
                pointerDrag.onMove(delta => {
                    if (pointerLock.isLocked()) {
                        ...
                        if (this.controls.phi > -7 || (this.controls.phi <= -7 && delta.y > 0)) {
        ...
        this.third.load.texture('./assets/img/phaser-logo.png').then(grass => {
            ...
            this.ground = this.third.physics.add.ground({ width: 20, height: 20, y: -1 }, { phong: { map: grass, transparent: true } });
        });
    ...
    getGroundPointer() {
        if (this.ground) {
            ...
            raycaster.setFromCamera({x: .1, y: .1}, this.third.camera);
    ...
    getMouseoverObjects() {
        ...
        if (intersection.length > 0) {
            ...
            if (obj != undefined) {
                ...
                this.curSelection = this.third.add.box({x: Math.floor(x-1), y: Math.ceil(y)});
                console.log(x, y);
            }
        }
    }
    ...
    updateMovement() {
        ...
        if (this.orc && this.orc.body) {
            ...
            if (this.keys.w.isDown || this.move) {
                const x = Math.sin(theta) * moveSpeed,
                    y = this.orc.body.velocity.y,
                    z = Math.cos(theta) * moveSpeed;
